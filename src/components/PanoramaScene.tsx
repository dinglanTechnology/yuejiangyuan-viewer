...
interface PanoramaSceneProps {
  ...
  onHotspotClick?: (payload: { id: string; imageUrl: string; title?: string }) => void
}
...
function HotspotDom({ position, title, imageUrl, onClick }: { position: THREE.Vector3; title?: string; imageUrl: string; onClick?: () => void }) {
    ...
    <Html position={position} sprite center zIndexRange={[150, 250]}>
      <div
        ...
        style={{
          ...
          boxShadow: '0 10px 30px rgba(0,0,0,0.35), 0 0 0 8px rgba(255,255,255,0.08)',
        ...
        onMouseEnter={(e) => {
          e.currentTarget.style.boxShadow = '0 12px 36px rgba(0,0,0,0.45), 0 0 0 10px rgba(110,231,183,0.25)'
          e.currentTarget.style.border = '1px solid rgba(255,255,255,0.5)'
        }}
        onMouseLeave={(e) => {
          e.currentTarget.style.boxShadow = '0 10px 30px rgba(0,0,0,0.35), 0 0 0 8px rgba(255,255,255,0.08)'
      ...
      >
        <div
          style={{
            ...
            height: 36,
            borderRadius: 999,
            background: 'radial-gradient(ellipse at center, rgba(110,231,183,0.45), rgba(110,231,183,0) 60%)',
            filter: 'blur(10px)',
            pointerEvents: 'none'
          }}
        />
...
export default function PanoramaScene({ imageUrl, imageHotspots, onHotspotClick }: PanoramaSceneProps) {
      ...
      {imageHotspots?.map((hs) => {
        ...
        return (
          <HotspotDom
            ...
            onClick={() => onHotspotClick?.({ id: hs.id, imageUrl: hs.imageUrl, title: hs.title })}
          />
        )
      })}
    </mesh>
